...
<div class="card card-outline card-primary">
    <div class="card-body">
                ...
                <tbody>
                    ...
                    while ($row = $olist->fetch_assoc()):
                        ...
                        <tr>
                            ...
                            <td>
                                <?php echo $row['product_name'] . " ({$row['size']}) " ?>
                            </td>
                            <td class="text-right">
                                <?php echo number_format($row['price']) ?>
                            </td>
                            <td class="text-right">
                                <?php echo number_format($row['price'] * $row['quantity']) ?>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
        ...
        <div class="row" id="print-order">
            <div class="col-6">
                <p>Payment Method:
                    <?php echo $payment_method ?>
                </p>
                <p>Payment Status:
                    <?php echo $paid == 0 ? '<span class="badge badge-light text-dark">Unpaid</span>' : '<span class="badge badge-success">Paid</span>' ?>
                </p>
            </div>
            <div class="col-6 row row-cols-2">
                ...
                <?php if (!isset($_GET['view'])): ?>
                    <div class="col-3"></div>
                    <div class="col">
                        <button type="button" id="update_status" class="btn btn-sm btn-flat btn-primary">Update
                            Status</button>
                    </div>
                <?php endif; ?>
    ...
    <div class="modal-footer" id="order-print-buttons">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick="printContent('print-order')">Print</button>
    </div>
</div>
...
<script>
    ...
    function printContent(print_content) {
        ...
        document.getElementById("order-print-buttons").style.display = "none";
        ...
        document.getElementById("order-print-buttons").style.display = "block";
    }

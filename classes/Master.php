...
class Master extends DBConnection
{
	...
	function save_supply()
	{
		...
		// $check = $this->conn->query("SELECT * FROM `supplies` where `supplier` = '{$supplier}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
		// if ($this->capture_err())
		...
		// }
		if (empty($id)) {
			$sql = "INSERT INTO `supplies` set {$data} ";
			$save = $this->conn->query($sql);
		} else {
			$sql = "UPDATE `supplies` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if ($save) {
			$resp['status'] = 'success';
			if (empty($id))
				$this->settings->set_flashdata('success', "New Supply successfully saved.");
			else
				$this->settings->set_flashdata('success', "Supply successfully updated.");
		} else {
			$resp['status'] = 'failed';
	...
	function save_supplier()
	{
		...
		$check = $this->conn->query("SELECT * FROM `suppliers` where `supplier` = '{$supplier}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
		if ($this->capture_err())
			return $this->capture_err();
		if ($check > 0) {
			$resp['status'] = 'failed';
	...
	function save_scategory()
	{
		...
		$check = $this->conn->query("SELECT * FROM `services_category` where `category` = '{$category}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
		if ($this->capture_err())
			return $this->capture_err();
		if ($check > 0) {
			$resp['status'] = 'failed';
	...
	function delete_scategory()
	{
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `services_category` where id = '{$id}'");
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Category successfully deleted.");
		} else {
	...
	function save_ssub_category()
	{
		...
		$check = $this->conn->query("SELECT * FROM `services_subcat` where `sub_category` = '{$sub_category}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
		if ($this->capture_err())
			return $this->capture_err();
		if ($check > 0) {
			$resp['status'] = 'failed';
		...
		if (empty($id)) {
			$sql = "INSERT INTO `services_subcat` set {$data} ";
			$save = $this->conn->query($sql);
		} else {
			$sql = "UPDATE `sub_categories` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
	...
	function delete_ssub_category()
	{
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `services_subcat` where id = '{$id}'");
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Sub Category successfully deleted.");
		} else {
	...
	function save_service()
	{
		...
		$check = $this->conn->query("SELECT * FROM `services` where `service_name` = '{$service_name}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
		if ($this->capture_err())
			return $this->capture_err();
		if ($check > 0) {
			$resp['status'] = 'failed';
		...
		if ($save) {
			$upload_path = "uploads/service_" . $id;
			if (!is_dir(base_app . $upload_path))
				mkdir(base_app . $upload_path);
			if (isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0) {
				foreach ($_FILES['img']['tmp_name'] as $k => $v) {
	...
	{
		extract($_POST);
		$update = $this->conn->query("UPDATE `services_bookings` set `status` = '$status' where id = '{$id}' ");
		if ($update) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata("success", " Order status successfully updated.");
		} else {
...
switch ($action) {
	...
	case 'delete_category':
		...
		break;
	case 'save_scategory':
		echo $Master->save_scategory();
		break;
	case 'delete_scategory':
		echo $Master->delete_scategory();
		break;
	case 'save_sub_scategory':
		echo $Master->save_ssub_category();
		break;
	case 'delete_sub_scategory':
		echo $Master->delete_ssub_category();
		break;
	...
	case 'update_service_status':
		echo $Master->update_service_status();
		break;
	case 'pay_order':
